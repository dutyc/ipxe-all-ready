...
class NvmetHostClient:
    """nvmet-host 宿主服务 HTTP 客户端（urllib 标准库，避免加依赖）。

    本组件是宿主服务的唯一调用方（绑定 localhost + Bearer token）。"""
...
class NvmetBackend:
    ...
    def scan(self) -> dict:
        names = [n for n in os.listdir(self.disk_dir) if n.lower().endswith((".img", ".iso"))]
        created, skipped = [], []
