...
* **Bulk rollout**: Power on multiple machines simultaneously, let them auto‑pool → bind them in bulk with the bind wizard → clone one by one in the WebUI → set default boot.
