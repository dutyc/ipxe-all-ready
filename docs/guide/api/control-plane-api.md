...
|---|---|
| `base_nqn` | The prefix of the default boot disk NQN (same disk selection rule) — C3 assembly: the disk NQN is generated by the Agent from the uniform template `base:worker_id.os`, consumed by the firmware as `sanboot nvme://<ip>:4420/${base-nqn}:<worker>.<os>` (re-assembled by the firmware = the authoritative disk-record value). Not returned when the disk record lacks `nqn` (legacy disk; no derivation — legacy environments are not supported) or when the Worker has no system disk. |
