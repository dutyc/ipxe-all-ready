...
- **工具栏**：
  - 注册窗口面板：部署期开启窗口（5/15/30/60 分钟，到期自动关闭）后，新设备上报携公钥自动入池并完成密钥认领；关闭后只上报不入池。面板内还含「设备身份验签强制」开关（开启后仅验签通过的已绑定设备可引导）。
...
- **列表**：按入池时间（`first_seen`）排序；MAC 旁有复制按钮；状态（`pooled` 池中 / `bound` 已绑定 / `revoked` 已注销）、绑定 Worker、指纹摘要、来源（`ipxe` 窗口期入池 / `manual` 手动）、首次上报时间。
...
- **窗口期自动入池**：注册窗口开启期间，机器通电（PXE 引导）即携公钥自动入池并完成密钥认领，无需人工登记。
- **手动入池**：Devices 页「注册设备」/「登记设备入池」录入（窗口关闭时使用）。
...
| 机器通电但设备池没有新设备 | ① 注册窗口是否开启（Devices 页顶部面板）；② Operations 页看是否有 `device.register` 记录（窗口关闭时只上报不入池） |
